          ...
          >
            <template #default="scope">
              ...
              <template v-if="column.render">
                <component :is="column.render" :row="scope.row" :index="scope.$index + 1" />
              </template>
...
const getDataList = async () => {
  ...
  const pageInfo = customPage
    ? {
        [customPage || 'pageNum']: paginationInfo.pageNum,
        [customPageSize || 'pageSize']: paginationInfo.pageSize
      }
    : { ...paginationInfo }

  try {
    const response: any = await ajax(httpApi, convertParams({ ...pageInfo, ...(params || {}) }), method || 'get')
    const res: any = ajax ? response : response.data || {}

    loading.value = false
    if (res?.code === 200) {
      formFeildVal.dataSource = res?.rows || []
      dataAllTotal.value = res?.total || 0
...
const handlePagination = () => {
  if (!props.httpRequest?.pageNum && !props.httpRequest?.pageSize) return
  const pageNum = props.httpRequest?.pageNum || 1
